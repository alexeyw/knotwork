...
object WorkspaceNamePolicy {
    ...
    enum class Violation {
        /** The path carries a character [isForbidden] rejects. */
        CONTROL_CHARACTER,
    ...
    fun violationOf(relativePath: String): Violation? = when {
        relativePath.any(::isForbidden) -> Violation.CONTROL_CHARACTER
    ...
    fun replaceForbidden(name: String): String = buildString(name.length) {
        name.forEach { append(if (isForbidden(it)) REPLACEMENT else it) }
    }
    ...
    fun escapeForbidden(text: String): String {
        if (text.none(::isForbidden)) return text
        return buildString(text.length + ESCAPE_HEADROOM) {
            text.forEach { c ->
                when {
                    ...
                    c == '\r' -> append("\\r")
                    c == '\t' -> append("\\t")
                    isForbidden(c) -> append(unicodeEscape(c))
                    else -> append(c)
                }
            }
        }
